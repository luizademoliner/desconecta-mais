...
public interface ISessaoFocoRepository extends JpaRepository<SessaoFocoEntity, Long> {
    ...
    //busca a sessão mais recente (OrderByDataInicioDesc) daquele usuário, filtrando só por status EM_ANDAMENTO.
    // O findFirst garante que, mesmo que existisse mais de uma linha com esse status por algum bug,
    // a busca não quebra — ela simplesmente pega a primeira.
    Optional<SessaoFocoEntity> findFirstByUsuarioAndStatusOrderByDataInicioDesc(UsuarioEntity usuario, StatusSessaoFocoEnum status);
